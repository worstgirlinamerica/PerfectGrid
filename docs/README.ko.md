...
Perfect Grid는 로컬 동영상 파일로 동영상 콘택트 시트와 미리보기 썸네일을 생성할 수 있는 무료 오픈 소스 데스크톱 앱입니다. 동영상을 불러오고 레이아웃을 선택한 후 고화질 PNG 콘택트 시트로 내보내세요.

편집자, 수집가, 아카이브 담당자, 그리고 동영상의 빠른 시각적 요약이 필요한 모든 분을 위해 제작되었습니다.
...
<p align="center">
  ...
  <a href="https://github.com/worstgirlinamerica/PerfectGrid/releases/latest/download/PerfectGrid-v0.1.2-Linux-x86_64.AppImage">
    <img src="https://custom-icon-badges.demolab.com/badge/Download-Linux-E95420?logo=linux&logoColor=white" alt="리눅스용 다운로드">
  </a>
</p>
...
- 선택적 타임코드 오버레이
- **Refine Picks**를 통한 스마트 프레임 선택
- 사용자 지정 사전 설정 저장 및 재사용
- 일괄 처리
- Windows, macOS 및 Linux에서 실행 가능
> __어떤__ 오류가 발생하거나, 기능이 제대로 작동하지 않거나, 문의 사항이 있는 경우 [이슈](https://github.com/worstgirlinamerica/PerfectGrid/issues/new)를 생성하여 신고해 주시기 바랍니다.

## 현지화

앱 UI는 8개 언어로 제공됩니다. 시트 파일명 렌더링은 별도의 시스템으로, 내보낸 PNG 파일에 파일명을 픽셀로 그려 넣기 때문에 스크립트별 글꼴 처리가 필요합니다.
...
> 현재 리눅스에서 비-ASCII 파일 이름 표시는 지원되지 않습니다. 라틴 문자가 아닌 스크립트로 된 파일 이름은 빈 사각형으로 표시됩니다. 다음 릴리스에서 수정될 예정입니다!
...
5. (선택 사항) **범위** 탭에서 **선택 항목 다듬기**를 사용하여 더 정확하게 프레임을 선택하세요.
6. **범위**에서 내보내기 화질을 선택하세요: **빠름 (1080p)**, **세부 (1440p)** 또는 **최대 (4K)**.
7. **PNG 내보내기**를 클릭하세요.

## 설치

[릴리스](https://github.com/worstgirlinamerica/PerfectGrid/releases/latest) 페이지에서 사용자의 OS에 맞는 최신 버전을 다운로드하세요.
...
> macOS는 서명이 되어 있지 않은 앱이므로 첫 실행 시 앱을 차단합니다. 마우스 오른쪽 버튼을 클릭 → **열기** → **열기**를 선택하여 차단 메시지를 우회하세요. 그래도 열리지 않으면 터미널에서 다음 명령을 실행하세요: `xattr -cr "/Applications/Perfect Grid.app"`

### Windows

1. `PerfectGrid-v0.1.2-Windows-x86_64.zip`을 다운로드하세요.
2. 마우스 오른쪽 버튼을 클릭하고 **모두 추출**을 선택한 다음, 폴더를 엽니다.
3. `Perfect Grid.exe`를 더블 클릭합니다.

> 앱에 코드 서명이 되어 있지 않아 Windows SmartScreen에서 경고가 표시될 수 있습니다. **자세히 보기** → **어쨌든 실행**을 클릭하세요.

### 리눅스

1. `PerfectGrid-v0.1.2-Linux-x86_64.AppImage`를 다운로드합니다.
2. 실행 권한을 부여하고 실행합니다:
```bash
...
별도의 설치가 필요하지 않습니다. FFmpeg 및 모든 종속성이 포함되어 있습니다. 대부분의 x86_64 배포판(Ubuntu 22.04 이상, Fedora, Arch 등)에서 작동합니다.

> FUSE 오류가 발생하면 `sudo apt install fuse` (데비안/우분투) 또는 `sudo dnf install fuse` (페도라)를 실행하세요.

## 개인정보 보호

모든 처리는 로컬에서 이루어집니다. 분석, 원격 측정, 파일 업로드가 없습니다. FFmpeg와 FFprobe가 포함되어 있습니다.

## 참고 사항

- MP4, MOV, MKV, AVI, WebM 및 FFmpeg가 읽을 수 있는 모든 형식을 지원합니다.
...
빌드 스크립트는 `scripts/` 디렉터리에 있습니다. GitHub Actions는 태그가 지정된 푸시가 있을 때 Windows 및 Linux 릴리스를 자동으로 빌드합니다. macOS 버전은 수동으로 빌드해야 합니다.
...
포함하면 가장 유용한 정보는 사용 중인 OS, 비디오 형식, 그리고 문제가 미리보기, 다듬기, 내보내기 또는 일괄 처리 단계 중 어디에서 발생했는지입니다.
