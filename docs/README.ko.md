...
<p align="center">
  <img src="https://img.shields.io/github/v/release/worstgirlinamerica/PerfectGrid?color=E95420" alt="GitHub 릴리스">
...
편집자, 수집가, 아카이브 담당자, 그리고 동영상의 빠른 시각적 요약을 원하는 모든 분을 위해 제작되었습니다.
...
5. (선택 사항) **범위** 탭에서 **선택 항목 다듬기**를 사용하여 더 정교하게 프레임을 선택하세요.
...
> macOS는 서명이 되어 있지 않은 앱이므로 첫 실행 시 앱을 차단합니다. 마우스 오른쪽 버튼을 클릭 → **열기** → **열기**를 선택하여 차단 해제하세요. 그래도 열리지 않으면 터미널에서 다음 명령을 실행하세요: `xattr -cr "/Applications/Perfect Grid.app"`
...
> 앱에 코드 서명이 되어 있지 않아 Windows SmartScreen에서 경고가 표시될 수 있습니다. **추가 정보** → **어쨌든 실행**을 클릭하십시오.
...
별도의 설치가 필요하지 않습니다. FFmpeg 및 모든 종속성이 번들로 포함되어 있습니다. 대부분의 x86_64 배포판(Ubuntu 22.04 이상, Fedora, Arch 등)에서 작동합니다.

> FUSE 오류가 발생하면 `sudo apt install fuse`(데비안/우분투) 또는 `sudo dnf install fuse`(페도라)를 실행하세요.

## 개인정보 보호

모든 작업은 로컬에서 수행됩니다. 분석, 원격 측정, 파일 업로드가 없습니다. FFmpeg과 FFprobe가 포함되어 있습니다.

## 현지화

앱 UI는 8개 언어로 제공됩니다. 시트 파일명 렌더링은 별도의 시스템으로, 내보낸 PNG 파일에 파일명을 픽셀로 그려 넣기 때문에 스크립트별 폰트 처리가 필요합니다.
...
> 현재 Linux에서 비 ASCII 파일 이름 표시는 지원되지 않습니다. 라틴 문자가 아닌 문자 집합으로 된 파일 이름은 상자로 표시됩니다. 이 문제는 다음 릴리스에서 수정될 예정입니다!

## 참고 사항

- MP4, MOV, MKV, AVI, WebM 및 FFmpeg가 읽을 수 있는 모든 형식을 지원합니다.
- 구형 하드웨어에서는 AV1 및 VP9 디코딩 속도가 느릴 수 있습니다.
- macOS 빌드는 서명이 되어 있지 않습니다 — 위의 설치 참고 사항을 확인하세요.
...
빌드 스크립트는 `scripts/` 디렉터리에 있습니다. GitHub Actions는 태그가 지정된 푸시 시 Windows 및 Linux 릴리스를 자동으로 빌드합니다. macOS는 수동으로 빌드됩니다.
